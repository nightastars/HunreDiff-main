...
class Rmse(nn.Module):
    def __init__(self, conv, n_feats, depth, act=nn.ReLU(True)):
        super(Rmse,self).__init__()
        self.resblock = conv(n_feats, n_feats, kernel_size=3, padding=1, stride=1)
        self.sig = nn.Sigmoid()
        self.conv =nn.Sequential(conv(n_feats, n_feats, kernel_size=1), act,
